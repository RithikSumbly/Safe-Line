...
### Accessibility modes (Default vs Accessibility)

Frontend preferences are **two modes only** (not a long settings panel):

| Mode | Storage | Behavior |
|------|---------|----------|
| Default | `safeline-a11y-mode=default` | Existing brand UI; Desk/Coffee still independent |
| Accessibility | `safeline-a11y-mode=accessibility` | `html[data-a11y="accessibility"]` — higher contrast tokens, larger type, Atkinson Hyperlegible, stronger focus, underlined links, min touch targets, animations forced off |

Code: `contexts/A11yModeContext.tsx`, `components/AccessibilityModeToggle.tsx`, boot script in `index.html`, CSS in `index.css`. Audit/report: [`ACCESSIBILITY_REPORT.md`](ACCESSIBILITY_REPORT.md).

### WhatsApp verdict length

`whatsapp/formatter.py`: verdicts fit Meta’s ~4096 text limit (`_fit_whatsapp`, ~4000). Old hard cap of 1500 was removed after mid-source truncations; boilerplate “Running a live check…” is omitted when a full verdict is present.
...
| `contexts/ThemeContext.tsx` | Desk / Coffee brand theme |
| `contexts/A11yModeContext.tsx` | Default / Accessibility mode |
| `components/AccessibilityModeToggle.tsx` | A11y mode control (nav + sidebar) |
| `components/ThemeToggle.tsx` | Desk / Coffee control |
...
| `lib/chatImageCache.ts` | Guest session screenshot cache |
...
*Last synced July 2026 — safety-question routing, pending reply-vs-check UI, screenshot OCR + `image_data_url` persistence, WhatsApp interactive menus + ~4000-char formatter, WCAG Default/Accessibility modes (Lighthouse a11y 100 on primary routes), HF↔Vercel relay (`send` / `send_message` / `download_media`), LangGraph unused / custom orchestrator. Statements not directly observable in code are marked "could not be confirmed" above.*
